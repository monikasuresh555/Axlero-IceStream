...
const FLINK_REST_URL = process.env.FLINK_REST_URL || "http://localhost:8081";
...
interface FlinkJobOverviewJob {
  jid: string;
  name: string;
  state: string;
}

interface FlinkJobOverviewResponse {
  jobs: FlinkJobOverviewJob[];
}
...
async function fetchFlinkPipelineStatus() {
  try {
    const res = await fetch(`${FLINK_REST_URL}/jobs/overview`);
    if (!res.ok) throw new Error(`Flink overview ${res.status}`);

    const data = (await res.json()) as FlinkJobOverviewResponse;
    const job = data.jobs?.[0];

    return {
      status: job?.state || "UNKNOWN",
      jobId: job?.jid || null,
      anomalyModeActive: false,
      flinkOverview: data,
    };
  } catch (e) {
    return {
...
function getNullTaxRatio(): number {
  if (totalRowsEvaluated === 0) return 0;
  return Number(((nullTaxCount / totalRowsEvaluated) * 100).toFixed(2));
}

function getQualitySuccessRate(): number {
  let successfulAssertions = 4;
  if (isAnomalyActive) successfulAssertions = 2;
...
function buildGreatExpectationsReport() {
  ...
  if (isAnomalyActive) {
    failures.push("tax_amount null ratio exceeded 2% safety threshold.");
    failures.push("Negative amount validation failed.");
  }
...
function processStreamingBatch() {
  ...
  if (isAnomalyActive) {
    invalidBatch = Math.floor(incomingEvents * randomUniform(0.08, 0.18));
  } else {
    invalidBatch = Math.floor(incomingEvents * randomUniform(0.002, 0.008));
  }
  ...
  if (isAnomalyActive) {
    nullTaxCount = Math.floor(incomingEvents * randomUniform(0.08, 0.18));
    negativeAmountCount = Math.floor(incomingEvents * randomUniform(0.01, 0.04));
  } else {
    nullTaxCount = Math.floor(incomingEvents * randomUniform(0.001, 0.008));
  ...
  if (isAnomalyActive) {
    ...
    icebergCommits += Math.max(1, Math.floor(validBatch / 10));
  } else {
    icebergCommits += Math.max(1, Math.floor(incomingEvents / 10));
  }
  ...
  if (isAnomalyActive) {
    lastErrorRate = Number(
      Math.max(nullTaxRatio, 2 + randomUniform(0, 2)).toFixed(2)
    );
  } else {
    lastErrorRate = Number(Math.max(0.05, randomUniform(0.1, 0.6)).toFixed(2));
  }

  kafkaLag = isAnomalyActive
    ? Math.min(999, kafkaLag + randomInt(1, 5))
    : Math.max(0, kafkaLag + randomInt(-3, 2));

  latencyMs = isAnomalyActive ? randomInt(25, 55) : randomInt(12, 22);
}
...
function createAnomalyIncident() {
  const incident: Incident = {
    ...
    severity: "CRITICAL",
    ruleViolated: "tax_amount null ratio exceeded 2% safety threshold",
...
app.get("/api/pipeline-status", async (_req: Request, res: Response) => {
  const flink = await fetchFlinkPipelineStatus();
  const reportPath = "/tmp/validation_report.json";

  let externalReport: any = null;

  if (fs.existsSync(reportPath)) {
    try {
      externalReport = JSON.parse(fs.readFileSync(reportPath, "utf-8"));
    } catch {
      externalReport = null;
    }
  }
  ...
  res.json({
    ...
    flinkOverview: flink.flinkOverview,
    error: (flink as any).error || null,
    greatExpectationsReport: externalReport || buildGreatExpectationsReport(),
      ...
      flink:
        flink.status === "UNKNOWN"
          ? isAnomalyActive
            ...
            : "OPERATIONAL"
          : flink.status,
      iceberg: isAnomalyActive ? "PROTECTED" : "OPERATIONAL",
      minio: "OPERATIONAL",
      greatExpectations: isAnomalyActive ? "DEGRADED" : "OPERATIONAL",
    },
  });
});
...
app.get("/api/metrics", (_req: Request, res: Response) => {
  ...
  res.json({
    throughput_events_per_sec: metrics.throughput,
    processed_total: metrics.processedTotal,
    error_rate: metrics.errorRate,
    dlq_count: metrics.dlqCount,
    checkpoint: metrics.icebergCommits,
    checkpoint_duration_ms: metrics.latencyMs,
    kafka_consumer_lag: metrics.kafkaLag,
    latency_ms: metrics.latencyMs,
    valid_events: metrics.validEvents,
    invalid_events: metrics.invalidEvents,
  });
});
...
app.get("/api/iceberg/snapshots", (_req: Request, res: Response) => {
  res.json([
    ...
    },
    {
      ...
      operation: "append",
      records_added: 100,
      total_records: Math.max(0, processedTotal - 100),
...
wss.on("connection", (ws: WebSocket) => {
  ...
  ws.send(
    JSON.stringify({
      ...
      metrics: getCurrentMetrics(),
      greatExpectationsReport: buildGreatExpectationsReport(),
      flinkOverview: null,
    })
  );
...
setInterval(() => {
  ...
  broadcast({
    ...
    pipeline: {
      status: isPipelineRunning ? "RUNNING" : "PAUSED",
      anomalyActive: isAnomalyActive,
      circuitBreaker: isAnomalyActive ? "ENGAGED" : "CLOSED",
    },
    greatExpectations: buildGreatExpectationsReport(),
  });
}, 1000);
...
server.listen(PORT, "0.0.0.0", () => {
  ...
  console.log("Real-time pipeline simulation started.");
});